...
import { sendWelcomeEmail } from '@/server/integrations/email/client';
import { type PlanTier, type PlanLimits } from '@/lib/pricing';
import type { JWT } from 'next-auth/jwt';
import { getPlanLimitsAsync } from '@/server/billing/pricing-server';

/** Sync user profile fields from a DB record to a JWT token (used for impersonation start/exit) */
function syncUserToToken(
  dbUser: { id: string; username: string | null; name: string | null; timezone: string; timezoneAutoDetect: boolean; bio: string | null; plan: string; role: string; onboardingCompleted: boolean; emailVerified: Date | null; password: string | null },
  token: JWT,
): void {
  token.id = dbUser.id;
  ...
  token.timezoneAutoDetect = dbUser.timezoneAutoDetect;
      ...
      if (trigger === 'update') {
        ...
        if (session.impersonateUserId && token.role === 'ADMIN') {
          ...
          if (targetUser) {
            ...
            syncUserToToken(targetUser, token);
            return token;
          }
        }
        ...
        if (session.exitImpersonation && token.originalAdminId) {
          ...
          if (adminUser) {
            syncUserToToken(adminUser, token);
